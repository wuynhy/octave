...
const ListStages = ({ stageFilter }) => {
  ...
  return (
    <>
        ...
        {stages?.length > 0 ? (
            stages.map((stage) => (
            <div key={stage.id} className="flex flex-col w-[250px] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm animate-slideup rounded-lg cursor-pointer">
                <div className="relative w-full h-300 group">
                    <img src={stage.cover_url} alt="cover" className="w-full h-full rounded-lg" />
                </div>
                <div className="mt-4 flex flex-col font-semibold text-lg text-white truncate">{stage.name}</div>
