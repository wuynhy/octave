...
async def create_stage(
    ...
    if len(host_stages) != 0:
        raise HTTPException(status_code=403, detail="Host can only create one stage")
...
async def update_stage(
    ...
    if participants is not None:
        ...
        for participant_id in participants_ids:
            ...
            if not success:
                raise HTTPException(
                    status_code=500, detail="Failed to add participant to stage"
                )
    return update
...
async def delete_stage(
    ...
    if stage is None:
        raise HTTPException(status_code=404, detail="Stage not found")
    if stage.host != host:
        raise HTTPException(status_code=403, detail="Permission denied, only host can delete stage")
...
async def join_stage(
    ...
    if stage.host == current_user:
        raise HTTPException(status_code=403, detail="Already a host, cannot join stage")
    
    participants = stage.participants or []
    if current_user in participants:
        raise HTTPException(status_code=403, detail="Already a participant, cannot join stage")
    try:
        repository.create_stage_participant(stage_id, participant_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail="Failed to join stage + " + str(e))
    return repository.get_one(stage_id)


@router.delete("/stages/{stage_id}/leave", response_model=Union[StageOut, Error])
async def leave_stage(
    ...
    if stage.host == user_data["username"]:
        raise HTTPException(status_code=403, detail="Host cannot leave stage, delete stage instead")
    
    participants = stage.participants or []
    if user_data["username"] not in participants:
        raise HTTPException(status_code=403, detail="Not a participant, cannot leave stage")
    try:
        repository.remove_participant(stage_id, participant_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail="Failed to leave stage + " + str(e))
    return repository.get_one(stage_id)


@router.get("/stages/participant/{participant_id}", response_model=List[Dict[str, Union[int, str]]])
async def get_stages_by_participant(
    participant_id: int,
    repository: StageRepository = Depends(StageRepository),
):
    stages = repository.get_stages_by_participant(participant_id)
    return stages